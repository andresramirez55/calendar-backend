...
import (
	...
	"calendar-backend/internal/infrastructure/config"
	"encoding/json"
	"errors"
	"fmt"
	"log"
...
func (s *NotificationService) SendEmailNotification(event *domain.Event, reminderType string) error {
	var subject string
	var body string
...
func (s *NotificationService) SendWhatsAppNotification(event *domain.Event, reminderType string) error {
...
func (s *NotificationService) SendNotification(event *domain.Event, reminderType string) error {
	var errs []error
...
func (s *NotificationService) SendFamilyNotifications(event *domain.Event, reminderType string) error {
...
func (s *NotificationService) sendFamilyEmailNotification(event *domain.Event, recipient *FamilyMember, selectedChildren []string, reminderType string) error {
	var subject string
	var body string
...
func (s *NotificationService) sendFamilyWhatsAppNotification(event *domain.Event, recipient *FamilyMember, selectedChildren []string, reminderType string) error {
	if s.cfg.TwilioAccountSID == "" || s.cfg.TwilioAuthToken == "" {
